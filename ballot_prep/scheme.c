...
int full_elgamal_example(void) {
    ...
    Elgamal_Gen(sk, pk, g, p);
		...
		mpz_import(plaintext, 1, MPZ_WORDS_ORDER, 1, MPZ_WORDS_ENDIANNESS, MPZ_NAILS, plaintext_ballot);
		...
		printf("cipher sizes: p %d c0 %d c1 %d\n", mpz_sizeinbase(plaintext, (1 << 8)), mpz_sizeinbase(ciphertext_0, (1 << 8)), mpz_sizeinbase(ciphertext_1, (1 << 8)));
    ...
    mpz_t pk1;
		...
		mpz_import(p1, 1, MPZ_WORDS_ORDER, 32, MPZ_WORDS_ENDIANNESS, MPZ_NAILS, &buffer[0]);
		mpz_import(g1, 1, MPZ_WORDS_ORDER, 32, MPZ_WORDS_ENDIANNESS, MPZ_NAILS, &buffer[32]);
		mpz_import(pk1, 1, MPZ_WORDS_ORDER, 32, MPZ_WORDS_ENDIANNESS, MPZ_NAILS, &buffer[64]);
		mpz_import(sk1, 1, MPZ_WORDS_ORDER, 32, MPZ_WORDS_ENDIANNESS, MPZ_NAILS, &buffer[96]);
...
int main(void)
{
		...
		mpz_set_str(array1[0], "7918324333004779287780879909121159911537551977796076554305607309994905870203", 10);
    ...
    mpz_export(data_out,
		...
		printf("%02X:%02X:%02X:%02X\n", data_out[32], data_out[1], data_out[2], data_out[3]);*/
		char hardcoded_p[32] = {	0x7B, 0x9F, 0x4C, 0xA3, 0xF8, 0x8A, 0x0E, 0x1F,
															0x4A, 0x8C, 0xEE, 0x10, 0xBE, 0x72, 0x1E, 0x2B,
															0x78, 0xAC, 0x50, 0xE0, 0x1B, 0x92, 0x1C, 0x96,
															0x9D, 0xF5, 0xF1, 0x30, 0xDD, 0x9C, 0x81, 0x11 };
