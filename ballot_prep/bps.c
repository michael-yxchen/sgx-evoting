...
int full_elgamal_example(void) {
  ...
  */
  ...
  mpz_t ciphertext_01;
  mpz_t ciphertext_11;
  mpz_t decrypted_ciphertext1;

  mpz_init(plaintext1);
  mpz_init(ciphertext_01);
  mpz_init(ciphertext_11);
  mpz_init(decrypted_ciphertext1);

  char plaintext_ballot1[1] = {0x04};
  mpz_import(plaintext1, 1, MPZ_WORDS_ORDER, 1, MPZ_WORDS_ENDIANNESS, MPZ_NAILS,
             plaintext_ballot1);

  gmp_printf("\nplaintext: \n%Zd", plaintext1);
  Elgamal_encrypt(ciphertext_01, ciphertext_11, plaintext1, pk1, g1, p1);
  gmp_printf("\nciphertext: \n%Zd\n%Zd ", ciphertext_01, ciphertext_11);
...
int ballot_prep(void) {
  FILE *fp;
  char buffer[102400];
  char plaintext_choices[10];
  ...
  if (fp) {
    ...
    json_object_object_get_ex(parsed_json, "1", &ballot);

    num_selections = json_object_array_length(ballot);
    for(i=0; i<num_selections; i++) {
      item = json_object_array_get_idx(ballot, i);
      json_object_object_get_ex(item, "question", &question);
      printf("\n[BPS]: Question %d: %s\n", i, json_object_get_string(question));
      json_object_object_get_ex(item, "choice", &choices);
      printf("[BPS]: Choices %d: %s\n", i, json_object_get_string(choices));

      int num_choices = json_object_array_length(choices);
      printf("[BPS]: Select your candidate from 1-%d\n    -> ", num_choices);

      int n;
      while((scanf("%d",&n) != 1) || (n > num_choices || n < 0))
      {
          printf("    -> ");
          while(getchar() != '\n');
      }
      printf("[BPS]: You selected %d.\n", n);

      plaintext_choices[i] = n & 0xFF;
      //for(j=0; j<json_object_array_length(choices); j++) {
      //  choice = json_object_array_get_idx(choices, j);
      //  printf("%s", json_object_get_string(choice));
      //}

    }
  }
  else {
    printf("[BPS]: File does not exist.\n[BPS]: Exiting.\n");
    return 1;
  }
  ...
  for (size_t count = 0; count < sizeof pval/sizeof *pval; count++) {
      sscanf(pos, "%2hhx", &pval[count]);
      pos -= 2;
  }
  ...
  for (size_t count = 0; count < sizeof gval/sizeof *gval; count++) {
      sscanf(pos, "%2hhx", &gval[count]);
      pos -= 2;
  }
  ...
  for (size_t count = 0; count < sizeof pkval/sizeof *pkval; count++) {
      sscanf(pos, "%2hhx", &pkval[count]);
      pos -= 2;
  }
  ...
  /* Exports ciphertext into bytearray */
  mpz_export(&ct[0], &buf_sz, MPZ_WORDS_ORDER, 32, MPZ_WORDS_ENDIANNESS,
             MPZ_NAILS, ciphertext_0);
  mpz_export(&ct[32], &buf_sz, MPZ_WORDS_ORDER, 32, MPZ_WORDS_ENDIANNESS,
             MPZ_NAILS, ciphertext_1);
