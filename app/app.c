...
bool load_init_files(const char *const ballot_file,
                     const char *const admin_file,
                     const char *const voter1_file,
...
int main(int argc, char **argv) {
  ...
  if (!opt_keygen && !opt_sign && !opt_quote && !opt_admin && !opt_init &&
      !opt_open && !opt_cast && !opt_close && !opt_tally) {
  ...
  if (opt_init &&
      (!opt_enclave_path || !opt_sealedkey_file || !opt_adminkey_file ||
       !opt_ballot_file || !opt_bulletin_file || !opt_voter1_file ||
       !opt_voter2_file || !opt_voter3_file || !opt_sealedelection_file)) {
    fprintf(stderr, "UsageInit:\n");
    fprintf(stderr,
            "  %s --init --enclave-path --adminkey secp256r1.pem --ballot "
            "ballot.txt --bulletin bulletin.txt --voter1 alice.pem "
            "--voter2 john.pem --voter3 justin.pem\n",
            argv[0]);
    return EXIT_FAILURE;
  }
  ...
  bool success_status =
      ...
      (opt_admin ? enclave_get_elgamal_buffer_sizes()
      ...
      (opt_init
           ? load_init_files(opt_ballot_file, opt_adminkey_file,
                             opt_voter1_file, opt_voter2_file, opt_voter3_file)
           : true) &&
