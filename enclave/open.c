...
sgx_status_t ecall_open(char *cmd, size_t cmd_size, char *signature,
  ...
  }
  {
    ...
    if (unsealed_election_state_size != sizeof(*election_state)) {
      print("[TrustedApp][OPEN]: sealed election state size mismatch !\n");
      ret = SGX_ERROR_INVALID_PARAMETER;
      goto cleanup;
    }
  }
  ...
  if (election_state == NULL) {
    print("[TrustedApp][OPEN]: malloc(election_state) failed !\n");
  ...
  if ((ret = sgx_unseal_data((sgx_sealed_data_t *)sealed_election_state_buffer,
    ...
    print("[TrustedApp][OPEN]: sgx_unseal_data() failed !\n");
    goto cleanup;
  }

  // Verify signature on ballot
  print("[TrustedApp][OPEN]: verify_signature started !\n");
  {
    uint8_t result = 255;
    if ((ret = verify_signature((uint8_t *)cmd, (uint32_t)cmd_size,
      ...
      print("[TrustedApp][OPEN]: verify_signature failed !\n");
      goto cleanup;
    }
    printf("[TrustedApp][OPEN]: Signature verification result: %s\n", result == SGX_EC_VALID ? "True" : "False");
  }
  ...
  print("[TrustedApp][OPEN]: Completed.\n");
  ret = SGX_SUCCESS;
